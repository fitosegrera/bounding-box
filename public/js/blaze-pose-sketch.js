    ...
    for (var i = 0; i < poses.length; i++) {
      for (var j = 0; j < poses[i].keypoints3D.length; j++) {
        if (poses[i].keypoints3D[j].score > 0.5) {
          ...
          console.log(poses[i].keypoints3D[j].name);

          // Draw keypoint3D position of nose
          ...
          if (poses[i].keypoints3D[j].name == "rightEyeInner") {
            fill(255, 200, 200);
            fontSize(20);
            text(
              String(Math.round(posX3D * 100)) +
                ", " +
                String(Math.round(posY3D * 100)) +
                ", " +
                String(Math.round(posZ3D * 100)),
              0,
              0
            );
          }
          ...
          translate(-windowWidth / 2.0, -windowHeight / 2.0);
          translate(
            (posX3D * windowHeight) / 4.0,
            (posY3D * windowHeight) / 4.0,
            (posZ3D * windowHeight) / 16.0
          );
          normalMaterial();
          box(5);
